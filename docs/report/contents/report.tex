...
\begin{itemize}
  \item \textbf{核心库 \texttt{pcseg\_core}}：\texttt{Vec3}、\texttt{PointCloud}、\texttt{KdTree}、
        法向量估计、\texttt{Segmentation}、\texttt{PointCloudIO}、以及基于抽象基类
        \texttt{Primitive} 的合成点云生成器；
...
\begin{table}[!htb]
  ...
  \begin{tabular}{@{}lp{9.5cm}@{}}
    ...
    \texttt{Primitive}   & \textbf{抽象基类}：几何体接口（纯虚 \texttt{sample}/\texttt{name}） \\
    \quad\texttt{PlanePrimitive} & 派生类：平面（地面、斜面） \\
    \quad\texttt{SpherePrimitive} & 派生类：球面 \\
    \quad\texttt{CylinderPrimitive} & 派生类：圆柱侧面 \\
...
\subsection{面向对象设计：抽象、继承与多态}
为体现面向对象的抽象与多态，本项目在合成点云模块设计了一处自然的继承体系：
每一种几何体（平面、球、圆柱）都是一个 \texttt{Primitive}。\texttt{Primitive} 是
\textbf{抽象基类}，声明两个\textbf{纯虚函数}——\texttt{name()} 报出名称、\texttt{sample()}
在表面采样若干点并打标签；\texttt{PlanePrimitive}、\texttt{SpherePrimitive}、
\texttt{CylinderPrimitive} 各自\textbf{继承}它并给出不同实现。场景生成函数 \texttt{makeScene}
把各几何体对象统一存进一个“指向基类的智能指针”数组，循环调用它们的 \texttt{sample}
方法，运行时由\textbf{多态}（虚函数动态绑定）决定实际采样哪种几何体。

这样设计的理由是：各几何体的采样公式不同，但“在表面取点、加噪声、打标签”的流程是一致的。
用抽象基类统一接口后，新增一种几何体（如圆锥）只需再派生一个子类、在场景里多放一个对象即可，
\texttt{makeScene} 主体无需改动，符合开闭原则；同时把“地面/斜面”这类同构对象用同一个
\texttt{PlanePrimitive} 类、不同构造参数来表达，避免了重复代码。其 UML 类图见\autoref{fig:uml}。

\begin{figure}[!htb]
  \centering
  \begin{tikzpicture}[
      umlclass/.style={rectangle split, rectangle split parts=3, draw,
                       font=\scriptsize, text width=3.7cm, fill=blue!4},
      >={Triangle[open]}]
    \node[umlclass] (base) at (0,0) {
      \textbf{Primitive}〈抽象〉
      \nodepart{second}\# label\_ : int
      \nodepart{third}+ name() : string〈纯虚〉\\ + sample(\dots)〈纯虚〉\\ \# emit(\dots)
    };
    \node[umlclass] (plane) at (-5.0,-3.2) {
      \textbf{PlanePrimitive}
      \nodepart{second}- origin\_, du\_, dv\_ : Vec3
      \nodepart{third}+ name()\\ + sample(\dots)
    };
    \node[umlclass] (sphere) at (0,-3.2) {
      \textbf{SpherePrimitive}
      \nodepart{second}- center\_ : Vec3\\ - radius\_ : float
      \nodepart{third}+ name()\\ + sample(\dots)
    };
    \node[umlclass] (cyl) at (5.0,-3.2) {
      \textbf{CylinderPrimitive}
      \nodepart{second}- cx\_, cy\_, radius\_, height\_ : float
      \nodepart{third}+ name()\\ + sample(\dots)
    };
    % 继承关系：空心三角箭头由派生类指向基类
    \draw[->] (plane.north) -- (base.south west);
    \draw[->] (sphere.north) -- (base.south);
    \draw[->] (cyl.north) -- (base.south east);
  \end{tikzpicture}
  \caption{\texttt{Primitive} 抽象基类与派生类的 UML 类图（空心三角箭头表示继承）}\label{fig:uml}
\end{figure}
...
下面给出几处关键实现。完整代码见附录~\ref{app:code} 与随附源码仓库。
...
\begin{table}[!htb]
  ...
  \begin{tabular}{@{}lp{8.5cm}c@{}}
    ...
    多态        & 基类指针调用派生类虚函数 \texttt{name}/\texttt{sample}，几何约束成立 & 通过 \\
...
\begin{enumerate}[label=(\arabic*)]
  \item \textbf{使用环节}：方案比较、代码重构、编译错误排查、测试用例检查以及
        \LaTeX{} 排版校对。
  \item \textbf{使用原则}：辅助工具只用于提高实现和检查效率；算法选择、类划分、
        参数设定、测试断言和实验结论均由本人理解后确认。
  \item \textbf{核对过程}：对 KD 树近邻结果与暴力搜索进行比较，对法向量估计、
        PLY 读写、区域生长分割和多态采样分别编写测试，避免只依赖生成结果。
  \item \textbf{问题反思}：最初把“两垂直相接平面应分成 2 段”作为硬性断言，
        实际测试中发现区域生长可能沿棱边附近的渐变法向发生合并。随后将该测试改为
        空间分离的两平面，并在局限性部分说明棱边泄漏问题。这个过程说明，
        代码实现可以借助工具提高效率，但结果判断和问题修正必须由开发者完成。
\end{enumerate}
